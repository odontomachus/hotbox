import sys
from collections import defaultdict
import struct
from time import sleep

import serial
from serial import SerialException

class RunStatus:
    __slots__ = ('countdown', 't1', 't2', 'dg', 'dt', 'part', 'state', 'cycle')
    def __init__(self):
        self.countdown = 0
        self.t1 = self.t2 = 0
        self.dg = 0
        self.dt = 0
        self.part = 0
        self.state = 0
        self.cycle = 0

class Client:
    """ Client class for hotbox serial connection """

    def __init__(self):
        self.queue = {
            'run_status': [],
            'status': [],
            'config': [],
        }

    def connect(self, port):
        try:
            self.conn = serial.Serial(port, 9600, timeout=0.5)
            # empty buffer
            while len(s.read(1)) > 0:
                pass
            self.state = 'connected'
        except SerialException:
            self.state = 'disconnected'

    def listen(self):
        s = self.conn
        keys = ['Time left', 'T1', 'T2', 'Off goal', 'Temp diff', 'Part', 'State', 'Cycle']
        print("\t".join(keys))
        out = open("hotbox.tsv", "w")
        while True:
            try:
                message = s.read(100)
                if len(message) == 0:
                    continue
                if message[0:4] != b"MSG:":
                    try:
                        print(message.decode('ascii'))
                        sleep(0.2)
                    except UnicodeDecodeError:
                        pass
                else:
                    row = defaultdict(str)
                    parse_message(message, row)
                    if row:
                        print("\t".join(map(lambda key: str(row[key]), keys)))
                        out.write("\t".join(map(lambda key: str(row[key]), keys))+"\r\n")
                        out.flush()
            except SerialException:
                self.state = 'disconnected'
            except KeyboardInterrupt:
                sys.exit()

    def parse_message(self, message, data):
        while len(message) > 4 and message[0:4] != b"MSG:":
            message = message[1:]
        if len(message) == 0:
            return False
        message = message[4:]
        while len(message) > 0:
            try:
                if message[0] == b'T'[0]:
                    data['T1'] = message[1]
                    data['T2'] = message[2]
                    message = message[3:]
                elif message[0] == b'C'[0]:
                    data['Time left'] = message[1]*256
                    data['Time left'] += message[2]
                    message = message[3:]
                elif message[0] == b'P'[0]:
                    data['Part'] = message[1]
                    message = message[2:]
                elif message[0] == b'Y'[0]:
                    data['Cycle'] = message[1]
                    message = message[2:]
                elif message[0] == b'S'[0]:
                    data['State'] = "On" if message[1] != '\x00' else "Off"
                    message = message[2:]
                elif message[0] == b'G'[0]:
                    data['Off goal'] = struct.unpack('b', message[1:2])[0]
                    message = message[2:]
                elif message[0] == b'D'[0]:
                    data['Temp diff'] = struct.unpack('b', message[1:2])[0]
                    message = message[2:]
                elif message == b":EOM":
                    return True
                else:
                    return False
            except:
                return False
                

    def configure(self, ctime, temp):
        self.conn.write(b'c'+struct.pack('B', ctime//256) + struct.pack('B', ctime%256) + struct.pack('B', temp))

    def start(self):
        self.conn.write(b's')

    def stop(self):
        self.conn.write(b't')

    def status(self):
        self.conn.write(b'p')

    def query(self):
        self.conn.write(b'q')

    def register(self, messages, fn):
        """ Register an action to do when a message is received.
        
        @param messages: list of message types to act on
        @param fn: callback to run when message is received
        """
        for message in messages:
            self.queue[message].append(fn)

    
    def message(self, mtype, message):
        """ Dispatch message to callbacks.
        
        @param mtype: message type
        @param message: (dict) message
        """
        for action in self.queue[mtype]:
            action(**message)
