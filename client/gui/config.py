...
class Config(tk.Frame):
    def __init__(self, done, controller, *args, **kwargs):
        ...
        tk.Spinbox(time_fr, exportselection=0, textvariable=self.v_h, width=3, from_=0, to=96).pack(side=tk.LEFT)
        tk.Label(time_fr, text=":").pack(side=tk.LEFT)
        tk.Spinbox(time_fr, exportselection=0, textvariable=self.v_m, width=3, from_=0, to=59, format="%02.0f").pack(side=tk.LEFT)
        tk.Label(time_fr, text=":").pack(side=tk.LEFT)
        tk.Spinbox(time_fr, exportselection=0, textvariable=self.v_s, width=3, from_=0, to=59, format="%02.0f").pack(side=tk.LEFT)
